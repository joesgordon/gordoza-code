package org.jutils.drawing;

import java.awt.Color;
import java.util.HashMap;
import java.util.Map;

import org.jutils.data.ColorMapType;
import org.jutils.drawing.HistogramView.HistogramConfig;

public class ColorModelFactory
{
    /**  */
    private final Map<ColorMapType, IModelCreator> maps;

    /***************************************************************************
     * 
     **************************************************************************/
    public ColorModelFactory()
    {
        this.maps = new HashMap<>();

        IModelCreator imc;

        imc = ( d ) -> createQuadraticMap( d );
        maps.put( ColorMapType.QUADRATIC, imc );

        imc = ( d ) -> createLinearStepMap( d );
        maps.put( ColorMapType.LINEAR_STEP, imc );

        imc = ( d ) -> createGrayscaleMap( d );
        maps.put( ColorMapType.GRAYSCALE, imc );

        imc = ( d ) -> createMatlabHotMap( d );
        maps.put( ColorMapType.MATLAB_HOT, imc );

        imc = ( d ) -> createMatlabDefault( d );
        maps.put( ColorMapType.MATLAB_DEFAULT, imc );

        imc = ( d ) -> createNightVision( d );
        maps.put( ColorMapType.NIGHT_VISION, imc );
    }

    /***************************************************************************
     * @param map
     * @return
     **************************************************************************/
    public IColorModel get( ColorMapType map, int pixelDepth )
    {
        IModelCreator imc = maps.get( map );

        if( imc == null )
        {
            return null;
        }

        return imc.createModel( pixelDepth );
    }

    /***************************************************************************
     * @return
     **************************************************************************/
    public static IColorModel createStdColorModel( int pixelDepth )
    {
        // Create a 6x6x6 color cube
        int [] seeds = new int[256];
        int i = 0;
        for( int r = 0; r < 256; r += 51 )
        {
            for( int g = 0; g < 256; g += 51 )
            {
                for( int b = 0; b < 256; b += 51 )
                {
                    seeds[i++] = ( r << 16 ) | ( g << 8 ) | b;
                }
            }
        }

        // And populate the rest of the cmap with gray values
        int grayIncr = 256 / ( 256 - i );

        // The gray ramp will be between 18 and 252
        int gray = grayIncr * 3;
        for( ; i < 256; i++ )
        {
            seeds[i] = ( gray << 16 ) | ( gray << 8 ) | gray;
            gray += grayIncr;
        }

        return new SeedColorModel( seeds, pixelDepth );
    }

    /***************************************************************************
     * @return
     **************************************************************************/
    public static IColorModel createGrayscaleMap( int pixelDepth )
    {
        int [] seeds = new int[256];
        int cnt = seeds.length;
        double grayIncr = 1;

        // The gray ramp will be between lower and upper
        double gray = 0.0;

        for( int i = 0; i < cnt; i++ )
        {
            int g = ( int )gray;
            seeds[i] = ( g << 16 ) | ( g << 8 ) | g;
            gray += grayIncr;
        }

        return new SeedColorModel( seeds, pixelDepth );
    }

    /***************************************************************************
     * @param lower
     * @param upper
     * @return
     **************************************************************************/
    public static IColorModel createMatlabHotMap( int pixelDepth )
    {
        int [] seeds = new int[] { 720896, 1376256, 2097152, 2818048, 3473408,
            4194304, 4849664, 5570560, 6291456, 6946816, 7667712, 8388608,
            9043968, 9764864, 10420224, 11141120, 11862016, 12517376, 13238272,
            13959168, 14614528, 15335424, 15990784, 16711680, 16714496,
            16717056, 16719872, 16722688, 16725248, 16728064, 16730624,
            16733440, 16736256, 16738816, 16741632, 16744448, 16747008,
            16749824, 16752384, 16755200, 16758016, 16760576, 16763392,
            16766208, 16768768, 16771584, 16774144, 16776960, 16776976,
            16776992, 16777008, 16777024, 16777040, 16777056, 16777072,
            16777088, 16777103, 16777119, 16777135, 16777151, 16777167,
            16777183, 16777199, 16777215 };

        return new SeedColorModel( seeds, pixelDepth );
    }

    /***************************************************************************
     * @param lower
     * @param upper
     * @return
     **************************************************************************/
    public static IColorModel createMatlabDefault( int pixelDepth )
    {
        int [] seeds = new int[] { 143, 159, 175, 191, 207, 223, 239, 255, 4351,
            8447, 12543, 16639, 20735, 24831, 28927, 33023, 36863, 40959, 45055,
            49151, 53247, 57343, 61439, 65535, 1114095, 2162655, 3211215,
            4259775, 5308335, 6356895, 7405455, 8454016, 9437040, 10485600,
            11534160, 12582720, 13631280, 14679840, 15728400, 16776960,
            16772864, 16768768, 16764672, 16760576, 16756480, 16752384,
            16748288, 16744448, 16740352, 16736256, 16732160, 16728064,
            16723968, 16719872, 16715776, 16711680, 15663104, 14614528,
            13565952, 12517376, 11468800, 10420224, 9371648, 8388608 };

        return new SeedColorModel( seeds, pixelDepth );
    }

    /***************************************************************************
     * @return
     **************************************************************************/
    public static IColorModel createQuadraticMap( int pixelDepth )
    {
        int [] seeds = new int[256];

        int r, g, b;
        double rd, gd, bd;
        double s = 0.02;

        for( int i = 0; i < seeds.length; i++ )
        {
            rd = -s * Math.pow( ( i - 224.0 ), 2.0 ) + 255.0;
            gd = -s * Math.pow( ( i - 128.0 ), 2.0 ) + 255.0;
            bd = -s * Math.pow( ( i - 32.0 ), 2.0 ) + 255.0;

            rd = Math.max( rd, 0 );
            gd = Math.max( gd, 0 );
            bd = Math.max( bd, 0 );

            rd = Math.min( rd, 255 );
            gd = Math.min( gd, 255 );
            bd = Math.min( bd, 255 );

            r = ( int )Math.round( rd );
            g = ( int )Math.round( gd );
            b = ( int )Math.round( bd );

            seeds[i] = r << 16 | g << 8 | b;
        }

        return new SeedColorModel( seeds, pixelDepth );
    }

    /***************************************************************************
     * @return
     **************************************************************************/
    public static IColorModel createLinearStepMap( int pixelDepth )
    {
        byte [] r = new byte[256];
        byte [] g = new byte[256];
        byte [] b = new byte[256];

        linearize( b, 0, 64, 0, 255 );
        linearize( b, 64, 128, 255, 0 );
        linearize( b, 128, 256, 0, 255 );

        linearize( g, 0, 128, 0, 255 );
        linearize( g, 128, 192, 255, 0 );
        linearize( g, 192, 256, 0, 255 );

        linearize( r, 0, 256, 0, 255 );

        int [] seeds = new int[256];

        for( int i = 0; i < seeds.length; i++ )
        {
            seeds[i] = Byte.toUnsignedInt( r[i] ) << 16;
            seeds[i] |= Byte.toUnsignedInt( g[i] ) << 8;
            seeds[i] |= Byte.toUnsignedInt( b[i] );
        }

        return new SeedColorModel( seeds, pixelDepth );
    }

    /***************************************************************************
     * @param lower
     * @param upper
     * @return
     **************************************************************************/
    private static IColorModel createNightVision( int pixelDepth )
    {
        int [] seeds = new int[256];

        int g = 0;
        int r = 0;
        int b = 0;

        double delta = 256.0 / seeds.length * 2.0;
        for( int i = 0; i < 128; i++ )
        {
            g = ( int )Math.round( i * delta );
            seeds[i] = ( g << 8 );
        }
        for( int i = 0; i < 128; i++ )
        {
            r = ( int )Math.round( i * delta );
            b = r;
            seeds[i + 128] = ( r << 16 ) | ( 255 << 8 ) | b;
        }

        return new SeedColorModel( seeds, pixelDepth );
    }

    /***************************************************************************
     * @param c
     * @param x1
     * @param x2
     * @param y1
     * @param y2
     **************************************************************************/
    private static void linearize( byte [] c, int x1, int x2, int y1, int y2 )
    {
        double v;
        double m = ( y2 - y1 ) / ( double )( x2 - x1 );
        double b = interp( x1, y1, x2, y2, 0.0 );

        for( int d = x1; d < x2; d++ )
        {
            v = m * d + b;

            c[d] = ( byte )( v );
        }
    }

    /***************************************************************************
     * @param x1
     * @param y1
     * @param x2
     * @param y2
     * @param x
     * @return
     **************************************************************************/
    public static double interp( double x1, double y1, double x2, double y2,
        double x )
    {
        return y1 + ( x - x1 ) * ( y2 - y1 ) / ( x2 - x1 );
    }

    /***************************************************************************
     * 
     **************************************************************************/
    private static interface IModelCreator
    {
        public IColorModel createModel( int pixelDepth );
    }

    /***************************************************************************
     * 
     **************************************************************************/
    private static final class SeedColorModel implements IColorModel
    {
        private final int [] seeds;
        private final double scale;

        private HistogramConfig histCfg;

        public SeedColorModel( int [] seeds, int pixelDepth )
        {
            this.seeds = seeds;
            int pixelMax = ( int )Math.pow( 2.0, pixelDepth );
            this.scale = seeds.length / ( double )pixelMax;

            this.histCfg = new HistogramConfig();
        }

        @Override
        public Color getColor( int pixel )
        {
            if( pixel < histCfg.lowThreshold.value )
            {
                return histCfg.lowThreshold.color;
            }
            else if( pixel > histCfg.highThreshold.value )
            {
                return histCfg.highThreshold.color;
            }

            int index = ( int )( scale * pixel );

            return new Color( seeds[index] );
        }

        @Override
        public void setContrast( int contrast )
        {
            // TODO Auto-generated method stub
        }

        @Override
        public void setLowThreshold( int pixel, Color thresholdColor )
        {
            histCfg.lowThreshold.value = pixel;
            histCfg.lowThreshold.color = thresholdColor;
        }

        @Override
        public void setHighThreshold( int pixel, Color thresholdColor )
        {
            histCfg.highThreshold.value = pixel;
            histCfg.highThreshold.color = thresholdColor;
        }

        @Override
        public void setHistogramConfig( HistogramConfig histConfig )
        {
            setLowThreshold( histConfig.lowThreshold.value,
                histConfig.lowThreshold.color );
            setHighThreshold( histConfig.highThreshold.value,
                histConfig.highThreshold.color );
            setContrast( histConfig.contrast );
        }

        @Override
        public HistogramConfig getHistogramConfig()
        {
            return histCfg;
        }
    }
}
